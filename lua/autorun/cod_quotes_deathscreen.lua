...
local remindermessage = CreateClientConVar("cod_death_spawn_reminder", 0, true, false, "Enable the respawn reminder after dying.", 0, 1)
...
if ( CLIENT ) then
	...
	local tbl_funny_quotes = {
		...
		"Don't worry, that didn't look too bad."
	}
	...
	hook.Add("entity_killed", "deathsystem", function(data)
        if deathsystem:GetBool() and data.entindex_killed == LocalPlayer():EntIndex() then
            local ply = LocalPlayer()
            local quote
            local quotes = {
                [0] = tbl_noquotes,
                [1] = tbl_callofduty_quotes,
                [2] = tbl_callofduty2_c2a_quotes,
                [3] = tbl_funny_quotes,
                [4] = tbl_info_quotes,
                [5] = tbl_cod_all_quotes
            }
            local quoteset = quotes[cod_quotes:GetInt()] or tbl_noquotes
            quote = quoteset[math.random(#quoteset)]
			...
			local fadetbl = { 4.3, 6, 5.25, 3, 4.5, 3, 5 }
			...
			if remindermessage:GetBool() then
